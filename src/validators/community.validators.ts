...
const communityCoverImageSchema = z
  ...
  .refine(
    (value) =>
      value.startsWith("illustration:") ||
      z.string().url().safeParse(value).success,
    "Cover image must be a valid URL or illustration token",
  );

const safeUserText = (schema: z.ZodString) =>
  schema.refine(
    isAllowedUserContent,
    "This text contains content that is not allowed",
  );
...
const milestoneSchema = z
  .object({
    ...
    description: safeUserText(
      z.string().max(500, "Description must be less than 500 characters"),
    ).optional(),
  ...
  .superRefine((milestone, context) => {
    ...
    if (
      milestone.sequenceEndDay !== undefined &&
