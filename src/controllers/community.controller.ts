...
interface TemplateMilestoneForSnapshot {
  name: string;
  points: number;
...
function getTemplateRewardMilestones(
  milestones: TemplateMilestoneForSnapshot[],
  targetDays: number,
): GoalRewardMilestoneInput[] {
  const rewards: GoalRewardMilestoneInput[] = [];
  for (const milestone of milestones) {
    if (milestone.triggerType === MilestoneTriggerType.PERCENTAGE) {
      rewards.push({
        name: milestone.name,
    ...
    if (milestone.triggerType === MilestoneTriggerType.DAY) {
      rewards.push({
        name: milestone.name,
        points: milestone.points,
...
function getTemplateScheduleFields(schedule: Record<string, unknown> | undefined) {
  if (!schedule) {
    return { scheduleType: GoalScheduleType.DAILY, weekdays: [] as number[] };
  }
  const type = String(schedule.type);
  if (type === "WEEKLY") {
    return {
      scheduleType: GoalScheduleType.WEEKLY,
      weekdays: [Number(schedule.weekday)],
    };
  }
  if (type === "SELECTED_WEEKDAYS") {
    return {
      scheduleType: GoalScheduleType.SELECTED_WEEKDAYS,
  ...
  return {
    scheduleType:
      type === "ONE_TIME" ? GoalScheduleType.ONE_TIME : GoalScheduleType.DAILY,
    weekdays: [] as number[],
  };
}

function getTemplateTargetFields(
  target: Record<string, unknown> | undefined,
  targetDays: number,
) {
  if (target?.type === "QUANTITY") {
    return {
      targetType: GoalTargetType.QUANTITY,
  ...
  if (target?.type === "UNTIL_DATE") {
    return {
      targetType: GoalTargetType.UNTIL_DATE,
      targetValue: targetDays,
      unit: null,
    };
  }
  return {
    targetType: GoalTargetType.CHECK_IN_COUNT,
  ...
  try {
    ...
    const rankedCommunities = communitiesForDiscovery
      ...
      .sort((left, right) => {
        ...
        return (
          right.community.updatedAt.getTime() -
...
export async function getCommunityById(req: AuthRequest, res: Response) {
  try {
    ...
    logger.error("Get community by ID error:", {
      error,
      userId: req.userId,
...
export async function updateCommunity(req: AuthRequest, res: Response) {
  try {
    ...
    logger.error("Update community error:", {
      error,
      userId: req.userId,
      communityId: req.params.communityId,
    });
  ...
  try {
    ...
    if (community) {
      notificationService
        ...
        .catch((err) =>
          logger.error("Error sending community deleted notification:", err),
        );
    }
...
export async function joinCommunity(req: AuthRequest, res: Response) {
  try {
    ...
    const memberName =
      member.user.username || member.user.firstName || "Someone";
    notificationService
      ...
      .catch((err) =>
        logger.error("Error sending member joined notification:", err),
      );
...
export async function leaveCommunity(req: AuthRequest, res: Response) {
  try {
    ...
    await communityActivityService.createMemberLeftActivity(
      communityId,
      userId,
    );
    ...
    if (leavingMember) {
      ...
      notificationService
        ...
        .catch((err) =>
          logger.error("Error sending member left notification:", err),
        );
    }
...
export async function updateMemberRole(req: AuthRequest, res: Response) {
  try {
    ...
    notificationService
      ...
      .catch((err) =>
        logger.error("Error sending role changed notification:", err),
      );
...
export async function createTemplate(req: AuthRequest, res: Response) {
  try {
    ...
    await communityActivityService.createTemplateCreatedActivity(
      template.id,
      userId,
      communityId,
    );
    ...
    const creatorName =
      template.creator.username || template.creator.firstName || "Someone";
    const community = await prisma.community.findUnique({
      where: { id: communityId },
      select: { name: true },
    });
    if (community) {
      notificationService
        .sendTemplateCreatedNotification(
          communityId,
          template.id,
          template.goalText || "",
          creatorName,
        ...
        .catch((err) =>
          logger.error("Error sending template created notification:", err),
        );
    }
...
export async function getTemplateById(req: AuthRequest, res: Response) {
  try {
    ...
    logger.error("Get template by ID error:", {
      error,
      userId: req.userId,
...
export async function updateTemplate(req: AuthRequest, res: Response) {
  try {
    ...
    const isCommunityOwnerOrMod = await isOwnerOrMod(
...
export async function getTemplateParticipants(req: AuthRequest, res: Response) {
  try {
    ...
    logger.error("Get template participants error:", {
      error,
      userId: req.userId,
      templateId: req.params.templateId,
    });
...
export async function deleteTemplate(req: AuthRequest, res: Response) {
  try {
    ...
    if (templateWithCommunity) {
      notificationService
        .sendTemplateDeletedNotification(
          templateId,
        ...
        .catch((err) =>
          logger.error("Error sending template deleted notification:", err),
        );
    }
...
export async function startGoalFromTemplate(req: AuthRequest, res: Response) {
  try {
    ...
    const startDate = DateTime.now().setZone(timezone).toISODate() ?? new Date().toISOString().slice(0, 10);
    let schedule: GoalScheduleInput;
    if (scheduleOverride) {
      schedule = scheduleOverride;
    } else if (template.scheduleType === GoalScheduleType.ONE_TIME) {
      schedule = { date: startDate, type: "ONE_TIME" };
    } else if (template.scheduleType === GoalScheduleType.WEEKLY) {
      schedule = {
        startDate,
        type: "WEEKLY",
        weekday: template.weekdays[0] ?? DateTime.fromISO(startDate).weekday,
      };
    } else if (template.scheduleType === GoalScheduleType.SELECTED_WEEKDAYS) {
      schedule = {
        startDate,
        type: "SELECTED_WEEKDAYS",
        weekdays: template.weekdays.length
          ? template.weekdays
          : [DateTime.fromISO(startDate).weekday],
      };
    } else {
    ...
    if (template.targetType === GoalTargetType.QUANTITY) {
      target = {
        amount: template.targetValue ?? template.targetDays,
        type: "QUANTITY",
        unit: template.unit ?? "units",
      };
    } else if (template.targetType === GoalTargetType.UNTIL_DATE) {
      target = {
        endDate:
    ...
    const rewards = getTemplateRewardMilestones(
      template.milestones,
      Math.max(1, template.targetDays),
    );
    const goal = await createGoalV2(
      userId,
      timezone,
      {
      ...
      },
      {
        ...
        rewardSource: GoalRewardPlanSource.COMMUNITY_TEMPLATE,
      },
    );
    ...
    if (template.createdBy !== userId) {
      ...
      notificationService
        .sendGoalStartedFromTemplateNotification(
          template.createdBy,
          starterName,
          template.goalText || "",
        ...
        .catch((err) =>
          logger.error("Error sending goal started notification:", err),
        );
    }
    ...
    if (handleSubscriptionAccessError(error, res)) return;
    logger.error("Start goal from template error:", {
      error,
      userId: req.userId,
      templateId: req.params.templateId,
    });
...
export async function reactToActivity(req: AuthRequest, res: Response) {
  try {
    ...
    if (activity.userId !== userId) {
      ...
      notificationService
        .sendActivityReactionNotification(
          activity.userId,
          reactorName,
          activity.type,
        ...
        .catch((err) =>
          logger.error("Error sending reaction notification:", err),
        );
    }
    ...
    logger.error("React to activity error:", {
      error,
      userId: req.userId,
...
export async function createComment(req: AuthRequest, res: Response) {
  try {
    ...
    if (activity.userId !== userId) {
      const commenterName =
        comment.user.username || comment.user.firstName || "Someone";
      notificationService
        .sendActivityCommentNotification(
          activity.userId,
          commenterName,
          text,
          activity.community.name,
...
export async function deleteComment(req: AuthRequest, res: Response) {
  try {
    ...
    const isOwnerOrModd = await isOwnerOrMod(
      (comment as any)?.activity?.communityId,
      userId,
    );
...
export async function getCommunityStats(req: AuthRequest, res: Response) {
  try {
    ...
    const [memberCount, templateCount, activeGoalCount, recentActivityCount] =
      await Promise.all([
...
async function findActiveDuplicateInvite(params: {
  ...
  return (
    invites.find(
      (invite) => invite.maxUses === -1 || invite.uses < invite.maxUses,
    ) || null
  );
}
  ...
  try {
    ...
    if (inviteeUserId) {
      notificationService
        ...
        .catch((err) =>
          logger.error("Error sending invite notification:", err),
        );
    }
    ...
    if (normalizedEmail) {
      emailService
        ...
        .catch((err) =>
          logger.error("Error sending community invite email:", err),
        );
    }
...
export async function joinByInviteCode(req: AuthRequest, res: Response) {
  try {
    ...
    logger.error("Join by code error:", {
      error,
      userId: req.userId,
...
export async function revokeInvite(req: AuthRequest, res: Response) {
  try {
    ...
    if (
      !(await isOwnerOrMod(invite.communityId, userId)) &&
