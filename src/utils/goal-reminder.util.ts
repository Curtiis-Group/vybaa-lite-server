...
export type GoalReminderCopy = {
  message: string;
  title: string;
};

export type GoalReminderOccurrence = {
  dayKey: string;
  scheduledFor: Date;
};
...
function getActionPhrase(goalTitle: string): string | null {
  const normalized = goalTitle.toLowerCase();
  const prefixedAction = normalized.match(
    /^(?:to|i want to|i need to|my goal is to)\s+(.+)$/,
  );
  if (prefixedAction?.[1]) return prefixedAction[1];

  const firstWord = normalized.split(/\s+/)[0];
  return ACTION_VERBS.has(firstWord) ? normalized : null;
}

export function buildGoalReminderCopy(params: {
  goalTitle: string;
  ...
  const preferredName =
    params.preferredName?.trim().split(/\s+/)[0]?.slice(0, 40) || "there";
  const actionPhrase = getActionPhrase(goalTitle);
  const variant = getDeterministicIndex(
    `${params.seed}:${preferredName}:${goalTitle}`,
    3,
  );
...
export function getNextGoalReminderOccurrence(params: {
  now: Date;
  reminderTime: string;
  timezone: string | null;
}): GoalReminderOccurrence | null {
  const timeMatch = params.reminderTime.match(/^([01]\d|2[0-3]):([0-5]\d)$/);
  if (!timeMatch) return null;

  const requestedTimezone = params.timezone?.trim() || "UTC";
  const localNow = DateTime.fromJSDate(params.now, {
  ...
  let occurrence = DateTime.fromObject(
    {
      day: localDay.day,
      hour,
      millisecond: 0,
  ...
  if (occurrence <= localNow) {
    ...
    occurrence = DateTime.fromObject(
      {
        day: localDay.day,
        hour,
        millisecond: 0,
