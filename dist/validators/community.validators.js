...
const communityCoverImageSchema = zod_1.z
    .string()
    .refine((value) => value.startsWith("illustration:") ||
    zod_1.z.string().url().safeParse(value).success, "Cover image must be a valid URL or illustration token");
const safeUserText = (schema) => schema.refine(content_moderation_util_1.isAllowedUserContent, "This text contains content that is not allowed");
// Community creation schema
exports.createCommunitySchema = zod_1.z.object({
    name: safeUserText(zod_1.z
        .string()
        .min(1, "Community name is required")
        .max(100, "Community name must be less than 100 characters")),
    description: safeUserText(zod_1.z.string().max(1000, "Description must be less than 1000 characters")).optional(),
    coverImage: communityCoverImageSchema.optional(),
    isPublic: zod_1.z.boolean().default(true),
    category: safeUserText(zod_1.z.string().max(50, "Category must be less than 50 characters")).optional(),
});
...
exports.updateCommunitySchema = zod_1.z.object({
    name: safeUserText(zod_1.z
        .string()
        .min(1, "Community name is required")
        .max(100, "Community name must be less than 100 characters")).optional(),
    description: safeUserText(zod_1.z.string().max(1000, "Description must be less than 1000 characters")).optional(),
    coverImage: communityCoverImageSchema.nullable().optional(),
    isPublic: zod_1.z.boolean().optional(),
    category: safeUserText(zod_1.z.string().max(50, "Category must be less than 50 characters"))
        .nullable()
        .optional(),
});
...
const milestoneSchema = zod_1.z
    ...
    description: safeUserText(zod_1.z.string().max(500, "Description must be less than 500 characters")).optional(),
    ...
    if (milestone.sequenceEndDay !== undefined &&
        milestone.sequenceEndDay < startDay) {
        context.addIssue({
...
exports.updateTemplateSchema = zod_1.z.object({
    goalText: safeUserText(zod_1.z
        .string()
        .min(1, "Goal text is required")
        .max(500, "Goal text must be less than 500 characters")).optional(),
    targetDays: zod_1.z
        .number()
        .int("Target days must be an integer")
        .min(1, "Target days must be at least 1")
...
exports.createInviteSchema = zod_1.z
    .object({
