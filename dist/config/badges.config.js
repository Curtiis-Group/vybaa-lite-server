...
exports.getBadgesForType = getBadgesForType;
 ...
 */
function getBadgeDefinition(type, milestone) {
    const badges = exports.BADGE_DEFINITIONS[type];
    if (!badges)
        return null;
 ...
 */
function getBadgesForType(type) {
    return exports.BADGE_DEFINITIONS[type] || [];
}
/**
 * Get all streak milestone numbers
 */
function getStreakMilestones() {
    return exports.BADGE_DEFINITIONS.streak_milestone.map((b) => b.milestone);
}
/**
 * Check if a number is a milestone
 */
function isStreakMilestone(day) {
...
function getAllBadgeDefinitions() {
    return Object.values(exports.BADGE_DEFINITIONS).flat();
}
